  ...
  componentDidMount() {
    ...
    this.db
      ...
      .then(() => {
        if (
          this.state.album &&
          sessionStorage.authCode === this.state.album.lastAuthId
        ) {
          this.setState({
            AuthDone: true,
          });
              ...
              <div className="h-50 w-75 card bg-dark border-light">
                <div className="card-body ">
                  <div className="d-flex flex-column">
                    <h3 className="row m-2 text-light">
                      {album.displayName + " " + album.albumType + " Album"}
                    </h3>
                    <h6 className="row mx-2 text-light">
                      This Album Requires a PassCode.
                    </h6>
                    <input
                      className="form-control m-2 bg-dark text-light"
                      type="password"
                      id="passCode"
                      placeholder="Enter the PassCode"
                    />
                    <small style={messageWarning} className="text-danger m-2">
                      {" "}
                      Seems like a Wrong passcode :({" "}
                    </small>
                    <div className="row justify-content-center">
                      <button
                        type="button"
                        className="btn btn-primary mx-2 my-3 w-50"
